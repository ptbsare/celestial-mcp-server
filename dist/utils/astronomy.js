...
export const DSO_CATALOG = new Map();
export const STAR_CATALOG = new Map();
export const COMMON_NAMES = new Map(); // Maps common names to catalog IDs
 ...
 */
export async function loadDSOCatalog(filePath) {
    try {
        ...
        await new Promise((resolve, reject) => {
            ...
            parser.on('data', (record) => {
                ...
                const pick = (x) => x !== undefined && String(x).trim() !== '' ? parseFloat(String(x)) : undefined;
                magnitude = pick(vMagStr);
                if (magnitude === undefined)
                    magnitude = pick(bMagStr);
                if (magnitude === undefined)
                    magnitude = pick(magStr);
                ...
                if (commonName)
                    COMMON_NAMES.set(commonName.toLowerCase(), String(name).toLowerCase());
            });
            parser.on('end', () => {
 ...
 */
export async function loadStarCatalog(filePath) {
    try {
        if (!fs.existsSync(filePath)) {
        ...
        await new Promise((resolve, reject) => {
            ...
            parser.on('data', (record) => {
                ...
                let raHours;
                let decDegrees;
                if (record.ra_hours !== undefined && record.ra_hours !== '')
                    raHours = parseFloat(record.ra_hours);
                else if (record.ra !== undefined && record.ra !== null && record.ra !== '')
                    raHours = parseFloat(record.ra);
                ...
                if (record.dec_degrees !== undefined && record.dec_degrees !== '')
                    decDegrees = parseFloat(record.dec_degrees);
                else if (record.dec !== undefined && record.dec !== null && record.dec !== '')
                    decDegrees = parseFloat(record.dec);
                else if (record.Dec !== undefined && record.Dec !== '')
                    decDegrees = parseFloat(record.Dec);
                if (!name || raHours === undefined || decDegrees === undefined || isNaN(raHours) || isNaN(decDegrees))
                    return;
                let magnitude;
 ...
 */
async function downloadCatalogIfNeeded(url, destPath, description, minSizeBytes = 500_000) {
    ...
    return new Promise((resolve) => {
        const doDownload = (attempt) => {
            ...
            const req = protocol.request(url, { method: 'GET', timeout: 30000 }, (response) => {
...
export async function initializeCatalogs() {
    ...
    if (!fs.existsSync(hygPath)) {
        ...
        downloadCatalogIfNeeded('https://raw.githubusercontent.com/astronexus/HYG-Database/master/hyg/CURRENT/hygdata_v41.csv', hygPath, 'HYG star database').then(success => {
    ...
    if (!fs.existsSync(ngcPath)) {
        ...
        downloadCatalogIfNeeded('https://raw.githubusercontent.com/mattiaverga/OpenNGC/master/database_files/NGC.csv', ngcPath, 'OpenNGC catalog').then(success => {
...
export function reloadCatalogsIfNeeded() {
    const __filename = fileURLToPath(import.meta.url);
    const __dirname = dirname(__filename);
...
export function convertToAltAz(coords, observer, date) {
    ...
    const hor = Astronomy.Horizon(date, astroObserver, coords.rightAscension, coords.declination, refraction);
    return {
 ...
 */
export function getObjectDetails(objectName, date, observer) {
    const normalizedName = objectName.toLowerCase();
    // Create astronomy-engine Observer reference for this function
    let astroObserver = new Astronomy.Observer(observer.latitude, observer.longitude, observer.elevation);
    ...
    if (equatorialCoords && !isSolarSystemObject) {
        ...
        Astronomy.DefineStar(Astronomy.Body.Star1, equatorialCoords.rightAscension, // RA in hours
        ...
        try {
            riseAstroTime = Astronomy.SearchRiseSet(Astronomy.Body.Star1, astroObserver, 1, startTime, 1);
        }
        catch (e) {
            riseAstroTime = null;
        }
        try {
            transitAstroEvent = Astronomy.SearchHourAngle(Astronomy.Body.Star1, astroObserver, 0, startTime, 1);
        }
        catch (e) {
            transitAstroEvent = null;
        } // Upper culmination
        try {
            setAstroTime = Astronomy.SearchRiseSet(Astronomy.Body.Star1, astroObserver, -1, startTime, 1);
        }
        catch (e) {
            setAstroTime = null;
        }
        try {
            lowerCulminationEvent = Astronomy.SearchHourAngle(Astronomy.Body.Star1, astroObserver, 12, startTime, 1);
        }
        catch (e) {
            lowerCulminationEvent = null;
        } // Lower culmination
    ...
    astroObserver = new Astronomy.Observer(observer.latitude, observer.longitude, observer.elevation);
 ...
 */
export function findNearbyStars(targetCoords, maxDistance, // in degrees
date) {
    const nearbyStars = [];
...
export function listCelestialObjects(category = 'all') {
    ...
    if (category === 'all' || category === 'planets') {
        result.push({
            ...
            objects: Object.keys(SOLAR_SYSTEM_OBJECTS).map(name => name.charAt(0).toUpperCase() + name.slice(1) // Capitalize first letter
            )
        });
    }
    if (category === 'all' || category === 'stars') {
        // Get unique star names and sort them
        const starNames = Array.from(new Set(Array.from(STAR_CATALOG.keys()).map(name => name.charAt(0).toUpperCase() + name.slice(1) // Capitalize first letter
        ))).sort();
        result.push({
            category: 'Stars',
            objects: starNames
...
export function calculateAngularSeparation(coords1, coords2) {
    ...
    const cosDeltaSigma = Math.sin(dec1Rad) * Math.sin(dec2Rad) +
...
export function calculateBearing(coords1, coords2) {
